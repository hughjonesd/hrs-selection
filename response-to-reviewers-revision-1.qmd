...
significance, always by bootstrapping the sample. TODO: do we always?
...
TODO: Tobias, can you check the relevant para? I guess that if both PGI and RLRS are
...
TODO: Could we look at health trends in phenotypes? I'd guess
...
TODO: can we figure out a test for this? 

Test: take difference between scores, times the sign of the low group score.
Then under the null we'd expect as many + as - differences. 

Really we should be doing the right test always! If abs size isn't enough...!
...
TODO: Write a paragraph on biol. selection. (Not sure what is meant here?) 
...
but we're open to suggestions to clarify what we say here. 
...
TODO: yes, I think that is probably fine. 
...
TODO: not sure about this. Possibly the guy is correct. The only time it matters
is when we care about absolute effect sizes. Then, using separate means is
like treating white people as a single population and asking how much means will
change in the white population.  Pooling means is like treating white people as
part of the whole US population. If we then make claims about how means for that
whole population will change, we are effectively assuming that effect sizes are 
the same across ethnicities, which seems unlikely.
