...
"chip-wide" scores. I hope this is OK.
...
Note that controlling for principal components in a regression, *y \~
PGIs + PCs*, gives the same effect sizes as running an uncontrolled
regression on PGIs which have previously had the PCs residualized out,
*y \~ PGIs_resid*. We control rather than residualizing because this
gives the correct standard errors[^1], which might matter with our
relatively low N.

[^1]: Because the controls are correctly accounted for when calculating
    degrees of freedom.

Why residualize or control for principal components at all? I think for
two reasons. First, we are ultimately interested in selection on the
"true PGI". Stripping out PCs is an admittedly imperfect way to control
for confounding population stratification. Second, PGIs are typically
residualized in this way, so by examining the effect of these we're
following the "industry standard" and learning about scores in the form
...
manuscript this was wrongly referred to as a correlation but has now
been updated. The polygenic scores are standardized to have a standard
deviation of 1. Under a univariate model, the regression coefficient is
the covariance divided by the variance in the independent variable.
Given the variance is 1, the regression coefficient is equal to the
covariance and thus the two are interchangeable.

The story is slightly complicated by the fact that we control for
principal components. Thus our estimates are the partial covariance
divided by the partial variance, the partial variance being slightly
less than 1. Slightly biasing the effect size upwards in magnitude.
Nevertheless, this is the standard approach used by others
[@kong2017selection].

It would be possible to residualise the polygenic score on the principal
components, before restandardizing it have a variance of 1. That we
might think we have something closer to the true polygenic score
unafflicted by population stratification, with the right variance. In
practice we've found this to only change estimates for the effect of the
EA PGI by \~1%. The area of our analysis where the exact estimate is key
is the error-corrected estimates. Since the estimates need to be
adjusted for the fact that the error-corrected polygenic scores will
covary differently with the principal components, making such a similar
adjustment would be rather difficult.

TE TODO: Tobias, can you check the relevant para? I guess that if both
PGI and RLRS are normalized then the covariance is the correlation; but
actually isn't the correct thing covariance with the not-normalized
RLRS?
...
the existing literature which is perhaps a bit over-focused on cognition
and IQ.

TE TODO: Could we look at health trends in phenotypes? I'd guess that
health trends improve over the C20, but we could look at some specifics.
(Depression/BMI?) One big issue here is that maybe the PGIs have
confounds... but perhaps that's less serious for some of these
phenotypes
...
Under the null, we'd expect the above equation to hold for just 50% of
the scores. If it holds for significantly more than 50% of the scores
...
largest and most precisely estimated selection effects in the whole
sample, we estimate selection effects in different subgroups and test
for a significant difference between them, via a simple interaction
term. We report these results too.
...
We have clarified the theory early on in the intro and abstract. TE
TODO: Write a paragraph on biol. selection. (Not sure what is meant
here?)
...
> and medical implications of natural selection, in addition to their
> social implications.
...
> population, with the caveat that this"one data point" may not be
> representative of the US population (still less the species!) The mean
> number of children of people born in each year was calculated using
> sampling weights."

Effectively, the white US population is a "single data point" for theory
testing. Note that since we don't compare the ethnicities, the only
difference this makes is in estimating the absolute size of effects.

TE TODO: change it to be separate RLRS baseline for white and black.
