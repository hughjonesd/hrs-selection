...
abstract: |
  ...
  parents and unmarried parents. Using respondents' number of 
...
The Robertson-Price Identity states that the change in a genetic trait between one generation and the next is equal to its covariance with relative fitness [@Price1970identity; @Robertson1966identity], assuming no mutations. In humans, infant mortality is so low today that a person's reproductive success is a very close approximation of the fitness. As such, the covariance between a polygenic score and RLRS yields the expected change in the mean polygenic score per generation. Because the polygenic scores are standardized to have a variance of 1, the regression coefficient of RLRS on the PGS is equal to the covariance. Our approach to measuring natural selection is not novel, having been employed in the literature before [@Beauchamp2016selection; @hugh2022human; @kong2017selection].
...
#|   Dashed lines are fitted from linear regressions.
...
We can also examine natural selection in the preceding and succeeding generations, by using reported number of siblings and grandchildren respectively. We regress PGS on respondents' number of living siblings in 2010. Data for dead siblings has too many missing values to use. We reweight respondents by the reciprocal of their number of siblings, to account for parents of many siblings being more likely to be a parent of a respondent. Parents of no siblings cannot be included, so coefficient sizes are not perfectly comparable across the generations. Appendix @fig-siblings plots coefficients on number of siblings versus coefficients on years of education. Correlations are significant and negative (correlation `r cors_sibs_edu$white[1]`, bootstrapped 95% C.I. `r ci_boot_sibs$white[1]` to `r ci_boot_sibs$white[2]`). There is a positive and significant correlation across generations, i.e. between PGS coefficients on number of siblings and number of children (`r gen_cor`, bootstrapped 95% C.I. `r ci_gen_cor[1,1]` to `r ci_gen_cor[1,2]`).

The HRS has data on the years of education for the parents of respondents. We recalculate the coefficients on education using values for the parents, in case the genetics of education and human capital changed between generations. We then correlate these new coefficients with the number of siblings, finding a positive correlation again (correlation `r cors_sibs_edu_parent$white[1]`, bootstrapped 95% C.I. `r ci_boot_sibs_parent$white[1]` to `r ci_boot_sibs_parent$white[2]`). The estimate is slightly smaller as would be expected since respondents likely recall their parent's education with some error.
...
To complement the above cross-PGS analysis, we focus on two scores which we a priori expect to show intergroup differences: educational attainment and ADHD. Both of these showed large differences in @hugh2022human, and have significant main effects in this data. Figure @fig-top-pgs-interactions displays coefficients of these scores on RLRS for each of the different groups. All differences are in the expected direction, with coefficients being larger for the disadvantaged group, except for low- vs- high-education and EA. However, the only significant difference is between the low- and high-income groups for the coefficient of EA, and this does not survive Bonferroni correction. \[XXX or we just say "nothing survives B. C.\]
...
run_interaction_model <- function (pgs, interaction) {
  ...
  cross_terms <- glue("{interaction} + {interaction}:{pgs}")
  form_interaction <- reformulate(cross_terms, response = "rlrs")
  rand_white <- rand |> subset(ethnicity == "white")
  mod_interaction <- svyglm_quiet(form_interaction, rand_white)
...
tidy_coefs |> 
  ...
  mutate(.by = c(pgs, interaction),
    ...
    Group = c("Disadvantaged", "Advantaged"),
  ...
  ggplot(aes(estimate, Split, colour = Group)) + 
    ...
    scale_fill_manual(values = c("Difference n.s." = "white", 
                                 "Difference p < 0.05" = "orange"),
                                 guide = guide_legend(title = NULL)) +
    scale_color_manual(values = c("Advantaged" = "steelblue3", 
                                  "Disadvantaged" = "red4")) +
    theme_minimal() +
    theme(
...
Across three generations of Americans, we found estimates of selection to correlate over time and that selection is always more positive for traits that negatively predict educational attainment. Patterns of selection appear consistent across the twentieth and early twenty-first century. The result is congruent with that of @Zietsch2014perfect, who found a perfect genetic correlation between number of children and number of grandchildren in a large sample of Swedish twins born between 1915-1929. The underlying causes of selection seem to have stayed the same over time, such that the effects of selection have accumulated.
...
#|   education. Dashed lines show linear regressions.
...
#|   relative to the success of other offspring. Dashed lines show linear regressions.
