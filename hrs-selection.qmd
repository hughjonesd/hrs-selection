...
abstract: |
  ...
  parents, but not among other groups. Using respondents' number of 
  siblings and grandchildren, we 
...
for (eth in eths) {
  ...
  cors_sibs_edu[[eth]] <- cor(tidy_pgs_sibs[[eth]]$estimate, 
    ...
    form_pgs_gkids <- map(pgs, 
                       \(x) reformulate(c(x, pcs), response = "rlrs_div_h13gkid"))
...
f_gkids <- map(pgs, \(x) reformulate(c(x, pcs), response = "rlrs_div_h13gkid"))
...
calc_delta_coefs <- function (w, data, term) {
  ...
  forms <-  map(cross_terms, 
                \(x) reformulate(c(x, term, pcs), response = "rlrs"))
...
# Is it ok to residualise PGS on PCS vs control for them?
# Frisch-Waugh-Lovell theorem: it's the same thing... But we need PGS standardized to sd of 1
# if the regression beta is to be the covariance for the Price Equation
# When you residualise on PCs *and then restandardize* then the beta *is different* than just controlling for PCs. The difference is marginal (~1% lower in the below example). Furthermore, controlling for PCs seems to have been the standard practice in Beauchamp and in Kong et al. 

# I'm not sure whether we can residualise on PCs then use the error correction because PGS true, will correlate differently with the PCs. Then again I'm not sure that the correction would be right for twin H2 since the PCs will surely only correlate with the common SNPs? I've consulted the Becker et al 2021 supplementary and it doesn't give advice on the use of PCs. I'd have to think very carefully about how to do this.

# Either we justify our approach as reasonable. Or we residualise on the PCs and go with it.
# I'm in favour of the latter, in part because explaining why the effect sizes switch sign in the error correction is awkward.

reg_simple <- lm(rlrs ~ scale(EA) ,rand$variables)
reg_pc <- lm(rlrs ~ scale(EA) + PC1_5A +PC1_5B +PC1_5C +PC1_5D +PC1_5E + 
             PC6_10A +PC6_10B +PC6_10C +PC6_10D +PC6_10E ,rand$variables)
# beta = -0.027651
reg_res_EA <- lm(`scale(EA)` ~ PC1_5A +PC1_5B +PC1_5C +PC1_5D +PC1_5E + 
             PC6_10A +PC6_10B +PC6_10C +PC6_10D +PC6_10E ,reg_pc$model)

reg_res_rlrs <- lm(rlrs ~ PC1_5A +PC1_5B +PC1_5C +PC1_5D +PC1_5E + 
             PC6_10A +PC6_10B +PC6_10C +PC6_10D +PC6_10E ,reg_pc$model)

reg_res_noscale <- lm(reg_res_rlrs$residuals ~ reg_res_EA$residuals, 
                               reg_pc$model)
# beta = -0.027651
reg2_res <- lm(reg_res_rlrs$residuals ~ scale(reg_res_EA$residuals),
                       reg_pc$model)
# beta = -0.027358

reg2_res_pgs_only <- lm(rlrs ~ scale(reg_res_EA$residuals),
                       reg_pc$model)
# beta = -0.027358

# Below I get the estimate of the interactions and their p values. 
# EA is only significant for income at p = 0.0106
# There are always traits significant after bonferroni. But usually it's autism
# Given DHJ's improvement to manuscript, I don't think we have anything more convincing to give by focusing on the top PGS.
