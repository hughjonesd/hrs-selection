...
abstract: |
  ...
  selected for (against). However, unlike in the UK, there is little evidence
  that selection effects are larger among low-income or less educated people.
date: "September 2023"
format: 
  pdf:
    keep-tex: true
...
calc_eth_cor <- function (weights, data) {
  ...
  cor(coef_pgs_black, coef_pgs_white)
}

cor_eths <- cor(tidy_pgs$white$estimate, tidy_pgs$black$estimate)
cor_eths_boot <- withReplicates(rand_boot, theta = calc_eth_cor)
ci_eths <- confint_workaround(cor_eths_boot)[1,]
```

I estimated effect sizes of PGS correlations with realised fertility among black and white respondents separately.[^1] Appendix @fig-fertility shows effect sizes for white respondents only; power is too low for individual PGS estimates to be informative in the black sample. Standard errors are large because of the relatively low sample sizes, and no scores are significant at Bonferroni-corrected p \< 0.05/`r n_tests`. But I am most concerned with looking at patterns across scores rather than judging the significance of individual scores.

[^1]: Throughout, I use "effect" as shorthand for "regression coefficient". No claim about causality is implied. Recall that natural selection involves correlation, not necessarily causation, between selected characteristics and fertility.
...
calc_cors_edu <- function (weights, data, var) {
  ...
  c(
    ...
    coef(lm(coef_pgs_fert ~ coef_pgs_edu))["coef_pgs_edu"]
  )
}
...
ci_boot_fert <- map(cors_fert_edu, \(x) confint_workaround(x)[1,])
ci_boot_sibs <- map(cors_sibs_edu, \(x) confint_workaround(x)[1,])

ci_boot_fert_lm <- map(cors_fert_edu, \(x) confint_workaround(x)[2,])
ci_boot_sibs_lm <- map(cors_sibs_edu, \(x) confint_workaround(x)[2,])

stopifnot(significant(ci_boot_fert$white))
stopifnot(insignificant(ci_boot_fert$black))
stopifnot(insignificant(ci_boot_sibs$white))
stopifnot(insignificant(ci_boot_sibs$black))
```

@fig-fertility-edu plots each PGS's partial correlation with fertility against its partial correlation with educational attainment. The relationship is negative among whites (correlation `r cors_fert_edu$white[1]`, bootstrapped 95% C.I. `r ci_boot_fert$white[1]` to `r ci_boot_fert$white[2]`) and negative but insignificant among blacks (correlation `r cors_fert_edu$black[1]`, bootstrapped 95% C.I. `r ci_boot_fert$black[1]` to `r ci_boot_fert$black[2]`). Bootstraps are used so as to make inferences from the sample of respondents (the different PGS are not a "sample" of anything).
...
for (eth in eths) {
  ...
  for (term in terms) {
    ...
    ci[[eth]][[term]] <- confint_workaround(diff_cor_ed[[eth]][[term]])
...
for (term in terms) {
  ...
  ci[[term]] <- confint_workaround(diff_effect_size[[term]])
...
Appendix @fig-x-edu shows essentially no difference in the distribution of absolute effect sizes between respondents with more and less education. Appendix @fig-x-inc, on the other hand, shows that absolute effect sizes appear larger among lower-income respondents. Appendix @fig-x-mar shows absolute effect sizes for respondents who were married in 2010 against all other statuses. Effect sizes appear larger for unmarried respondents. Appendix @fig-x-agefb splits respondents by age at first birth (median within each ethnicity). There are no large differences between early and late parents. Note that the N is lower here due to missing values. @tbl-x-bootstraps shows means and 95% confidence intervals for mean absolute effect sizes in the two groups, and the difference between them, using bootstraps. Differences for income and marriage are significant.
...
#| label: fig-fertility
#| fig-cap: |
#|  Effects of PGS on realized fertility among white respondents. 
#|  Values are partial correlations of a one standard deviation 
#|  (within ethnicity) change of the PGS with fertility, controlling 
#|  for 10 principal components of genomic array data. 
#| fig-height: 6
...
#| label: fig-x-edu
#| fig-cap: |
#|   Boxplots of polygenic score effects on fertility by education
#|   among white respondents. Effect sizes are
...
tidy_pgs_x_ed$white |> 
  ...
  ggplot(aes(Education, abs(estimate), color = Education)) +
    ...
    labs(
      ...
      title = paste("PGS effects on fertility by education", 
                    calc_n(mod_pgs_x_ed))
    ) +
    themes_x
```
...
#| label: fig-x-inc
#| fig-cap: |
#|   Boxplots of polygenic score effects on fertility by income
#|   among white respondents. Effect sizes are
#|   absolute sizes of effects of a one standard deviation (within-ethnicity) 
#|   change of the PGS on fertility, controlling for 10 principal 
#|   components of genetic array data, estimated within the low/high income
#|   group. Boxes show quartiles.
...
tidy_pgs_x_inc$white |> 
  ...
  ggplot(aes(Income, abs(estimate), color = Income)) +
    ...
    labs(
      x = "Income",
      y = "Absolute effect size",
      title = paste("PGS effects on fertility by income", 
...
#| label: fig-x-mar
#| fig-cap: |
#|   Boxplots of polygenic score effects on fertility by marital status
#|   among white respondents. Effect sizes are
#|   absolute sizes of effects of a one standard deviation (within-ethnicity) 
#|   change of the PGS on fertility, controlling for 10 principal 
#|   components of genetic array data, estimated within the low/high income
...
tidy_pgs_x_mar$white |> 
    ...
    ggplot(aes(married, abs(estimate), color = married)) +
      ...
      labs(
          x = "Married",
          y = "Absolute effect size",
          title = paste("PGS effects on fertility by marital status", 
...
#| label: fig-x-agefb
#| fig-cap: |
#|   Boxplots of polygenic score effects on fertility by age at 
#|   first birth (median split) among white respondents. Effect sizes are
#|   absolute sizes of effects of a one standard deviation (within-ethnicity) 
#|   change of the PGS on fertility, controlling for 10 principal 
#|   components of genetic array data, estimated within the low/high income
#|   group. Boxes show quartiles.
...
tidy_pgs_x_agefb$white |> 
    ...
    ggplot(aes(agefb, abs(estimate), color = agefb)) +
      ...
      labs(
          ...
          y = "Absolute effect size",
          title = paste("PGS effects on fertility by age at first birth", 
