...
for (eth in eths) {
  ...
  cor_fert_edu_phen[[eth]] <- 
    lm(raevbrn ~ raedyrs, data = r_eth, weights = weight) |> 
    coef() |> 
    pluck("raedyrs")
}
...
for (eth in eths) {
  rand_short <- rand |> 
    ...
    select(all_of(pgs), all_of(pcs), raevbrn, raedyrs, weight, r10sibs)
  cors_boot[[eth]] <-
    replicate(n_reps, {
      r_boot <- rand_short |> 
      ...
      coef_pgs_fert <- pgs |> 
        ...
        map(\(x) lm(x, data = r_boot, weights = weight)) |> 
        map_dbl(\(x) coef(x)[[2]])
      
      coef_pgs_sibs <- pgs |> 
        map(\(x) reformulate(c(x, pcs), response = "r10sibs")) |> 
        map(\(x) lm(x, data = r_boot, weights = weight/(r10sibs + 1))) |> 
        map_dbl(\(x) coef(x)[[2]])
