...
abstract: |
  ...
  selected for (against). Selection effects are larger among low-income
...
#| label: fig-rlrs-edu
#| fig-cap: |
#|   Scatterplot of PGS effect sizes on RLRS and years of
...
#| label: fig-rlrs
#| fig-cap: |
#|  Effects of PGS on RLRS among white respondents. 
#|  Values are partial correlations of a one standard deviation 
#|  (within ethnicity) change of the PGS with RLRS, controlling 
...
tidy_pgs$white |>
  ...
  ggplot(aes(estimate, term)) +
    ...
    labs(
      x = "", y = "",
      title = "Effects of polygenic scores on RLRS"
    ) +
    theme(
...
#| label: fig-x-edu
#| fig-cap: |
#|   Boxplots of polygenic score effects on RLRS by education
#|   among white respondents. Effect sizes are
#|   absolute sizes of effects of a one standard deviation (within-ethnicity) 
#|   change of the PGS on RLRS, controlling for 10 principal 
...
tidy_pgs_x_ed$white |> 
  ...
  ggplot(aes(Education, abs(estimate), color = Education)) +
    ...
    labs(
      ...
      title = paste("PGS effects on RLRS by education", 
                    calc_n(mod_pgs_x_ed))
    ) +
    themes_x
```
...
#| label: fig-x-inc
#| fig-cap: |
#|   Boxplots of polygenic score effects on RLRS by income
#|   among white respondents. Effect sizes are
#|   absolute sizes of effects of a one standard deviation (within-ethnicity) 
#|   change of the PGS on RLRS, controlling for 10 principal 
...
tidy_pgs_x_inc$white |> 
  ...
  ggplot(aes(Income, abs(estimate), color = Income)) +
    ...
    labs(
      x = "Income",
      y = "Absolute effect size",
      title = paste("PGS effects on RLRS by income", 
...
#| label: fig-x-mar
#| fig-cap: |
#|   Boxplots of polygenic score effects on RLRS by marital status
#|   among white respondents. Effect sizes are
#|   absolute sizes of effects of a one standard deviation (within-ethnicity) 
#|   change of the PGS on RLRS, controlling for 10 principal 
#|   components of genetic array data, estimated within the low/high income
...
tidy_pgs_x_mar$white |> 
    ...
    ggplot(aes(married, abs(estimate), color = married)) +
      ...
      labs(
          x = "Married",
          y = "Absolute effect size",
          title = paste("PGS effects on RLRS by marital status", 
...
#| label: fig-x-agefb
#| fig-cap: |
#|   Boxplots of polygenic score effects on RLRS by age at 
...
tidy_pgs_x_agefb$white |> 
    ...
    ggplot(aes(agefb, abs(estimate), color = agefb)) +
      ...
      labs(
          ...
          y = "Absolute effect size",
          title = paste("PGS effects on RLRS by age at first birth", 
