...
where $h^2$ is the heritability of the PGS target phenotype and $R^2_{PGS}$ is the $R^2$ of the measured PGS on the target phenotype [@becker2021resource]. I make this calculation using heritabilities from twin studies, and SNP- or chip-heritabilities, reported in @abdellaoui2021dissecting (chip-heritability is the total variance explained by common SNPs recorded by array chips).

@fig-true-effects reports estimates for the true PGS as well as the "true chip-PGS" - the true best predictor of the phenotype using only variants recorded on the array chip, for phenotypes where selection effects were nominally significant and a measure of the phenotype was available in-sample and was significantly predicted by the PGS. It's important to understand that estimates for the true PGS are more uncertain than for the true chip-PGS. Ideally, the true chip-PGS differs only from the recorded PGS because of sampling error in calculating the underlying summary statistics.[^2] The true PGS would use many more genetic variants, perhaps including e.g. *de novo* mutations, and the relationship between these variants' effect on fertility and on the target phenotype may not be the same as for genetic variants captured on array chips.

[^2]: But even the chip-PGS might contain non-random noise from correlated environments.

The HRS contains multiple PGS for several phenotypes, typically with the $R^2$ increasing over time. Appendix @fig-check-correction runs a sanity check in which I predict the coefficient of later scores on fertility, using earlier scores and the above formula. Mostly, earlier PGS do a good job at predicting the effect of later PGS on fertility, with the later PGS's point estimate within the earlier PGS's 95% confidence interval. One exception is that the point estimate of the earlier PGS for general cognition, GENCOG, on the effect of the later PGS GCOG2 has the wrong sign.

As a rough rule of thumb, a 0.1 standard deviation change in a polygenic score over a generation might count as "serious": about 54% of the new generation will be below the parents' mean. Many upper confidence bounds meet that threshold, but lower bounds are often small or include zero. Given the uncertainties about the exact nature of "noise" in existing PGS, the estimates -- even for chip heritability -- mostly show the limits of our knowledge, and should be treated as best guesses only.
...
results_df <- list_rbind(results_df)
```

```{r}
#| label: fig-true-effects
#| fig-cap: |
#|   Estimated effects on RLRS of true polygenic scores calculated
...
results_df |> 
  ...
  ggplot(aes(y = Phenotype, x = mean, xmin = conf.low, xmax = conf.high,
    ...
    labs(x = "Corrected effect on RLRS", y = "") +
    theme_minimal() 
...
#| fig-cap: |
#|   Estimated effects of series of PGS using errors-in-variables 
#|   correction. "Own" is the uncorrected estimate using the PGS
#|   itself. Other estimates are from earlier or later PGS targeting
#|   the same phenotype and corrected via the method in @becker2021resource.
#| fig-height: 6
...
results_df |> 
  ...
  mutate(
    ...
    Target = fct_relevel(target.pgs,
                         "ADHD_PGC17", "ADHD_PGC10", "GCOG2", "GENCOG",
                         "MDD2", "MDD", "EA3", "EDU3", "EDU2"),
  ...
  ggplot(aes(y = Target, x = mean, xmin = conf.low, xmax = conf.high,
    ...
    labs(x = "Estimated effect on RLRS with correction", y = "Target PGS") +
