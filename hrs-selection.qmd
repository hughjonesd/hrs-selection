...
# TODO: think about your bootstraps, what's the test... given that 
# the scores are not independent, does it really make sense to aggregate
# a general size comparison? Or am I secretly just reproducing my own
# crappy standard errors?
# 
# TODO: is ADHD reverse-coded? What about XDISORDER?
...
tails_boot_fert <- map(cors_fert_edu, 
                       \(x) x + c(-1.96, 1.96) * SE(x))

tails_boot_sibs <- map(cors_sibs_edu, 
                       \(x) x + c(-1.96, 1.96) * SE(x))
...
median_diff_effect <- function(w, data, term) {
  forms <- paste0(pgs, ":", term) |>
    map(\(x) reformulate(c(x, term, pcs), response = "raevbrn"))
  
  mods <- map(forms, \(x) lm(x, data, weights = w))
  cfs <- map(mods, coef)
  cross_terms <- map(cfs, \(x) x[str_detect(names(x), ":")])
  if(any(lengths(cross_terms) != 2)) stop("Extra cross terms")
  abs_cross_terms <- map(cross_terms, abs)
  diffs <- map_dbl(abs_cross_terms, diff) 
  median(diffs)
}

median_diff_ed <- median_diff_inc <- median_diff_mar <- median_diff_agefb <-
  list()
...
for (eth in eths) {
  ...
  median_diff_ed[[eth]] <- 
    withReplicates(rand_boot, theta = median_diff_effect, term = "edu")
  median_diff_inc[[eth]] <- 
    withReplicates(rand_boot, theta = median_diff_effect, term = "income_med")
  median_diff_mar[[eth]] <- 
    withReplicates(rand_boot, theta = median_diff_effect, term = "married")
  median_diff_agefb[[eth]] <-
    withReplicates(rand_boot, theta = median_diff_effect, term = "agefb")
}
...
ci_ed <- map(median_diff_ed, confint_workaround)
ci_inc <- map(median_diff_inc, confint_workaround)
ci_mar <- map(median_diff_mar, confint_workaround)
ci_agefb <- map(median_diff_agefb, confint_workaround)

# TODO: is median_diff really a sensible test to run? 
