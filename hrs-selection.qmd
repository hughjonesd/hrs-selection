...
The HRS has data on the years of education for the parents of respondents. We recalculate the coefficients on education using the parent's PGS, in case the genetics of education and human capital changed between generations. We then correlate these coefficients with the number of siblings, finding a significant result (correlation `r cors_sibs_edu_parent$white[1]`, bootstrapped 95% C.I. `r ci_boot_sibs_parent$white[1]` to `r ci_boot_sibs_parent$white[2]`). The correlation is only slightly smaller and the confidence intervals are only slightly wider. This is surprising since parent's education is likely recorded with error and the respondent's polygenic score is only a proxy for their parents.
