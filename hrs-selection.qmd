...
pgirep <- readr::read_tsv("data/pgi-repository/HRS_PGIrepo_v1.0.txt", show_col_types = FALSE)
names(pgirep)[1:2] <- c("hhid","pn")

# TE Maybe not the most efficient method, but this code removes MTAG PGS
pgirep <- select(pgirep,"hhid","pn",matches("PC"),matches("single"))

# TE Going to remove the NEB PGS here
pgirep <- select(pgirep, -c(PGI_NEBwomen_single) )

# TE this creates simple names for PGS in PGI Rep
pgi_rep_names <- str_split_fixed(names(pgirep)[23:57],"_",n=3)[,2]
names(pgirep)[23:57] <- pgi_rep_names
...
pgirep <- select(pgirep, -matches("^PC\\d"))
#|>
#|
pgen4e_new_names <- left_join(pgen4e_new_names, pgirep, by = join_by(hhid == hhid, pn == pn),suffix=c("","pgirep"))

# TE Height is a duplicated name, but it's with the old PGS, so fixing this manually
colnames(pgen4e_new_names)[which(names(pgen4e_new_names) == "HEIGHT")] <- "HEIGHTpgirep"
...
dupe_PGS <- c("EA3", # TE when I've tested it the pgirep one is slightly better...
              ...
              # we suppose it's equivalent to having CP and EA
              "BUNTE", #trans ethnic BUNTE is kept 
              "CKDTE", 
              "BMI2",
              "HEIGHT2",
              "CPD_GSCAN19",
              ...
              "ALC", #AUDIT is alcohol misuse in pgirep
 	           "EGFR",
	          "HBA1CEA" #African ancestry vs European ancestry. No TE
              )
...
pgen4e_new_names <- select(pgen4e_new_names,-hhid,-pn,-any_of(dupe_PGS),-any_of(old_pgs))
pgs_white <- names(pgen4e_new_names)
pgs_white[which(pgs_white=="HEIGHT")] <- "HEIGHTpgirep"
...
# TE 283 people report having grandkids, but having had no children. Weird!
gkids_unusable_white <- sum(na.omit(ifelse(subset(rand, ethnicity == "white")$h10gkid > 0 & subset(rand, ethnicity == "white")$raevbrn==0,1,0)))

gkids_unusable_black <- sum(na.omit(ifelse(subset(rand, ethnicity == "black")$h10gkid > 0 & subset(rand, ethnicity == "black")$raevbrn==0,1,0)))

# If grandkids but no kids div_gkid is infinite. Replacing with NA
rand$div_h10gkid[is.infinite(rand$div_h10gkid)] <- NA

# TE added some variables for grandkid analysis
# TE I've also added sampling weights in calculation of mean values
# note the child weight is just weight* number of kids. so mean_h10gkid 
# and mean_div_h10gkid are actually the same
rand <- rand |>
  mutate(.by = rabyear, # calculate this pooling ethnicity
    ...
    mean_div_h10gkid = mean(div_h10gkid*child_weight/mean_child_weight, na.rm = TRUE), 
...
# TE I've plotted the mean values by year. They look reasonably stable. Beauchamp 2016 took a three year average. we don't think this is necessary
...
# Child dataset testing
# TE sample size not too small... But mum and dad often report different values of grandkids
# and big questions of sampling bias. Given the weakness of the data I don't think the approach can be justified


#rand_male <- subset(rand,ragender == 1)
#rand_female <- subset(rand,ragender == 2)

#rand_mid <- merge(rand_male,rand_female, by = "hhid",suffixes=c(".m",".f"),all.x=FALSE,all.y=FALSE)
#rand_mid <- subset(rand_mid,ethnicity.m=="white"))
# 3588
  

#rand_mid$EA <- rand_mid$EA.m + rand_mid$EA.f
#install.packages("fixest")
#library(fixest)

#cor(rand_mid$EA3.f,rand_mid$EA3.m)
#reg <- lm(scale(EA3.f) ~ scale(EA3.m), data=rand_mid)
#reg <- feols(scale(EA3.f) ~ scale(EA3.m), data=rand_mid)
#reg <- feols(rlrs.m ~ EA3.m, data=rand_mid)
#rand_mid$gkid <- rand_mid$h10gkid.m/rand_mid$raevbrn.m
#rand_mid$gkidf <- rand_mid$h10gkid.f/rand_mid$raevbrn.f

#reg <- feols(gkid ~ EA, data=rand_mid, ~ child_weight.m)

#reg <- feols(gkidf ~ EA, data=rand_mid, ~ child_weight.f)


#reg <- feols(h10gkid.m ~ EA, data=rand_mid, ~ child_weight.m)
#reg <- feols(h10gkid.f ~ EA, data=rand_mid, ~ child_weight.f)
...
rand <- rand |> 
  select(ethnicity, all_of(pgs), all_of(old_pgs), 
         ...
         rabyear, born, raehsamp, raestrat, parent_weight, ragender,
         adhd_score, neurot, r10diabe, r10hearte, r10cesd, age_smoked,
         anxiety, r10cogtot, rlrs_div_h10gkid, child_weight)
# added my grandkid rlrs
...
# TE we don't think it makes sense to include black estimates in the calculation. Since we know PGS in blacks are extremely weak
# and the tests overlap near 1:1 with the polygenic scores in whites
# Could just report the results for whites? 
...
write.table(subset(rand$variables, ethnicity == "white"),"pgi_correct_data.txt",
            quote=FALSE, sep="\t")

correct_scores <- readxl::read_excel("PGI_Heritabilities.xlsx",na = "NA", range="B1:U53")
                            
# Where h2 and R2 are missing, we take values from a similar sample - the WLS
correct_scores$chip_h2 <- ifelse(is.na(correct_scores$HRS_h2) , correct_scores$WLS_h2,
                                 correct_scores$HRS_h2)

correct_scores$R2 <- ifelse(is.na(correct_scores$HRS_R2) , correct_scores$WLS_R2,
                                 correct_scores$HRS_R2)

correct_scores$twin_h2 <- correct_scores$TwinH2
correct_scores$pgs <- correct_scores$Abr

correct_scores[which(correct_scores[,"pgs"]=="CANNABIS"),"pgs"] <- "CANNABISpgirep"
correct_scores[which(correct_scores[,"pgs"]=="DPW"),"pgs"] <- "DPWpgirep"
correct_scores[which(correct_scores[,"pgs"]=="HEIGHT"),"pgs"] <- "HEIGHTpgirep"

# some of the PGS have estimates rounded to zero and are thus useless
correct_scores <- subset(correct_scores, chip_h2 > 0)
correct_scores <- subset(correct_scores, R2 > 0)


# TE I originally used 0.28 from the TED sample. But that leads to especially absurd estimates
# 0.15 was estiamted in the American MIDUS sample from the same paper. Leading to slightly less absurd estimates. Perhaps more appropriate in the American context. Uncertainty regarding parameters means results should be interepreted with CAUTION!!!
correct_scores[which(correct_scores$pgs=="AFB"),"twin_h2"] <- 0.15
#adjustment to use Midus values for NEB is also used
correct_scores[which(correct_scores$pgs=="NEBwomen"),"twin_h2"] <- 0.28

# PGI correct is quite fast, but can be made faster if the dataset is made smaller
#loop for chip results
map_df(1:nrow(correct_scores), function(i) {

  
system(paste('python3 pgi_correct/pgic.py --reg-data-file pgi_correct_data.txt --outcome "rlrs" --pgi-var',correct_scores[i,"pgs"],' --covariates "PC1_5A" "PC1_5B" "PC1_5C" "PC1_5D" "PC1_5E" "PC6_10A" "PC6_10B" "PC6_10C" "PC6_10D" "PC6_10E" --R2',correct_scores[i,"R2"],' --h2 ',correct_scores[i,"chip_h2"],' --weights "weight"',sep=" "))

system("sed '1d;2d;3d;4d;5d;6d;7d;8d;9d' pgi_correct.res > reg_results")
reg_snp <- read.table("reg_results",header=TRUE)

tibble(
  trait = as.character(correct_scores[i,"pgs"]), #Ugly way of removing the column name...
  beta = as.numeric(reg_snp[1,4]),
  SE = as.numeric(reg_snp[1,5]),
  Heritability = "Chip"
)
}
) ->
  corrected_effects_snp



# Loop for twin results
map_df(1:nrow(correct_scores), function(i) {


system(paste('python3 pgi_correct/pgic.py --reg-data-file pgi_correct_data.txt --outcome "rlrs" --pgi-var',correct_scores[i,"pgs"],' --covariates "PC1_5A" "PC1_5B" "PC1_5C" "PC1_5D" "PC1_5E" "PC6_10A" "PC6_10B" "PC6_10C" "PC6_10D" "PC6_10E" --R2',correct_scores[i,"R2"],' --h2 ',correct_scores[i,"twin_h2"],' --weights "weight"',sep=" "))

system("sed '1d;2d;3d;4d;5d;6d;7d;8d;9d' pgi_correct.res > reg_results")
reg_twin <- read.table("reg_results",header=TRUE)


tibble(
  trait = as.character(correct_scores[i,"pgs"]), 
  beta = as.numeric(reg_twin[1,4]),
  SE = as.numeric(reg_twin[1,5]),
  Heritability = "Twin"
)
}
) ->
  corrected_effects_twin

corrected_effects <- rbind(corrected_effects_snp, corrected_effects_twin)
corrected_effects$conf.low <- corrected_effects$beta - 1.96*corrected_effects$SE
corrected_effects$conf.high <- corrected_effects$beta + 1.96*corrected_effects$SE

# Deleting data created for PGI correct
system("rm pgi_correct_data.txt")




corrected_effects <- merge(corrected_effects, correct_scores, by.x="trait", by.y="Abr",prefix=c("",".y"))
...
corrected_effects$Category <- case_match(corrected_effects$Category, 
...
#570, 600
# or 5.98 and 6.79 inches

# This removes ADHD due to the low power of the score. Note ADHD predict RLRS after bonferroni! 
# It also removes NEB
corrected_effects <- subset(corrected_effects, R2 > 0.005)
...
# The low R2 of NEB causes it to be removed, but the genetics for NEB could just be a little different
# in HRS versus other samples!
#NEB_result <-  corrected_effects-which(corrected_effects[,"trait"]=="NEBwomen"),]
  ...
  theme_minimal() +
  facet_grid(Category ~ ., scales = "free_y", space = "free_y", labeller = my_labeller)+
  theme(strip.text.y = element_text(angle = -90,face = "bold"))
  
  corrected_effects$TwinH2_Source <- corrected_effects$Source
  corrected_effects <- corrected_effects[,c("trait","beta","SE","Heritability","chip_h2",
                                            "twin_h2","R2","TwinH2_Source","PoldermanICF_ICD10_subchapter","Category")]
  
 write.csv(corrected_effects,"Corrected_effects.csv")
