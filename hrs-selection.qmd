...
deltas |> 
  ...
  mutate(
    ...
    outliers = ifelse(rank(delta) %in% c(1:3, 66:68), PGS, ""),
    .by = group
  ) |> 
...
: Estimates and bootstrap 95% confidence intervals for numbers of PGS out of `r length(pgs_white)` where the disadvantaged group had a larger selection coefficient than the advantaged group. Groups are: 0-12 years education vs. 13-17 years; below vs. above median income; all others vs. married; below vs. above median age at first birth; born before vs. after 1942. `r n_reps_x_es` bootstraps.
