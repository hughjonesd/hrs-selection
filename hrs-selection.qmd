...
# TODO (related) move away from lists and towards a single datasource with an
# "ethnicity" column; results from broom::tidy can still be in lists?
...
calc_income <- function (x, eth) {
  ...
  income_r <- resid(lm(income ~ factor(x$rabyear), weights = x$weight))
...
for (eth in eths) {
  ...
  cors_boot[[eth]] <-
    replicate(n_reps, {
      # survey package automatically does the right thing
      # possibly I should be using survey::bootweights
      # or survey::withReplicates and a function
      r_boot <- rand_short[sample(nrow(rand_short), replace = TRUE), ]
      
      coef_pgs_fert <- pgs |> 
        map(\(x) reformulate(c(x, pcs), response = "raevbrn")) |> 
        map(\(x) svyglm(x, r_boot)) |> 
        map_dbl(\(x) coef(x)[[2]])
      
      coef_pgs_sibs <- pgs |> 
        map(\(x) reformulate(c(x, pcs), response = "r10sibs")) |> 
        # need to add , weights = weight/(r10sibs + 1)
        map(\(x) svyglm(x, r_boot)) |> 
        map_dbl(\(x) coef(x)[[2]])
